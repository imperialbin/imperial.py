...
class AsyncDocument(BaseDocument, ABC):
    ...
    async def edit(self, content: str = None, *,
        ...
        settings = await self.client.rest.request(method="PATCH", path="/document", payload=payload)
        self._update(**settings)

    async def duplicate(self, content: str, *,
        ...
        data = await self.client.rest.request(method="POST", path="/document", payload=payload)
        return AsyncDocument(self.client, **data)

    async def delete(self) -> None:
        await self.client.rest.request(method="DELETE", path=f"/document/{self.id}")
